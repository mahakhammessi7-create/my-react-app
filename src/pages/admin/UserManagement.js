        ...
        // expert_auditeur is intentionally EXCLUDED from this page
        const NON_EXPERT_ROLES = new Set([
        "admin","administrateur","charge_etude","charge-etude",
        "responsable","resp_suivi","decideur","client",
        ]);
        const normalize = u => ({
        id:       u.id,
...
export default function UserManagement() {
    ...
    <div className="page-header">
        ...
        <div style={{display:'flex', gap:'10px'}}>
          <button className="ancs-btn-back" onClick={() => navigate('/admin/dashboard')}>← Retour</button>
          <button className="btn-refresh" onClick={fetchUsers}>↻ Actualiser</button>
        </div>
    </div>  {/* Close the page-header div here */}
    {error && (
        ...
        {/* Rest of your code continues... */}
            ...
            <div className="section-card">
                ...
                {loading ? (
                <div className="loading-row"><div className="spinner"/><span>Chargement depuis Supabase…</span></div>
                ) : (
                <table className="ancs-table">
                    <thead>
                    <tr>{["Utilisateur","Organisme","Secteur","Rôle","Statut","Dernière connexion","Actions"].map(h=><th key={h}>{h}</th>)}</tr>
                    </thead>
                    <tbody>
        ...
        .ancs-btn-back{background:transparent;border:1px solid #30363d;color:#8b949e;padding:9px 18px;border-radius:6px;font-size:13px;cursor:pointer;font-family:'Inter',sans-serif;display:flex;align-items:center;gap:6px;}
        .ancs-btn-back:hover{border-color:#22d3ee;color:#22d3ee;}
