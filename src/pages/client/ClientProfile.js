...
const CSS = `
  ...
  .cp-edit-input:focus {
    border-color:#63d2be;
    box-shadow:0 0 0 3px rgba(99,210,190,.15);
  }
  .cp-edit-select {
  ...
  .cp-btn-modify:hover {
    ...
    border-color:rgba(99,210,190,.35);
  }
...
export default function ClientProfile() {
  useEffect(() => {
    injectStyles();
    return () => {
      const el = document.getElementById('cp-profile-styles');
      if (el) el.remove();
    };
  }, []);
  ...
  return (
    <div className="cp-root" style={{ minHeight:'100vh', background:'#07111e', color:'#e2f0ff' }}>
      ...
      <div className="cp-hex-bg" style={{ position:'relative', height:250, overflow:'hidden', background:'linear-gradient(155deg,#0c1f3a 0%,#0a2540 45%,#061520 100%)' }}>
        ...
        {/* Rotating rings */}
        {[
          { w:360, h:360, t:-120, r:-80,  d:22 },
          { w:200, h:200, t:30,   r:200,  d:16 },
          { w:140, h:140, t:90,   r:90,   d:30 },
          { w:480, h:480, t:-200, r:280,  d:40 },
        ].map((s,i) => (
      ...
      <div style={{ maxWidth:1080, margin:'0 auto', padding:'0 28px 64px' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', gap:26, marginTop:-76, marginBottom:38, flexWrap:'wrap' }}>
          ...
          <div style={{ flex:1, paddingBottom:4, animation:'fadeUp .5s ease .1s both' }}>
            ...
            <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
              ...
              <span style={{ background: userData.role==='admin'?'rgba(251,191,36,.09)':'rgba(99,210,190,.09)', color:userData.role==='admin'?'#fbbf24':'#63d2be', border:`1px solid ${userData.role==='admin'?'rgba(251,191,36,.22)':'rgba(99,210,190,.22)'}`, padding:'3px 12px', borderRadius:99, fontWeight:700, fontSize:10, letterSpacing:'.7px', textTransform:'uppercase' }}>
                {userData.role==='admin'?'👑 Admin':'🏢 Entreprise'}
              </span>
            </div>
          </div>
          ...
          <div style={{ paddingBottom:4, display:'flex', gap:10, animation:'fadeUp .5s ease .2s both' }}>
            {!editMode ? (
              <button className="cp-btn-modify" onClick={() => { setForm(userData); setEditMode(true); }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <button className="cp-btn-danger" onClick={() => { localStorage.clear(); window.location.href = '/'; }}>
            🚪 Se déconnecter
          </button>
        </div>
