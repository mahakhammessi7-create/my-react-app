...
export default function ChargeEtudeDashboard() {
  ...
  const getPublicPdfUrl = useCallback((rep) => {
    ...
    if (rep?.file_path?.startsWith('http')) return rep.file_path;
    if (rep?.file_path) {
      const { data } = supabase.storage
        .from('audit-reports')
        .getPublicUrl(rep.file_path);
      return data?.publicUrl || null;
    }
    return rep?.pdf_url || null;
  }, []);

  const reportWithUrl = fullReport 
    ? {
        ...fullReport,
        file_url: getPublicPdfUrl(fullReport),
      }
    : selectedReport 
      ? {
          ...selectedReport,
          file_url: getPublicPdfUrl(selectedReport),
        }
      : null;

  const handleNewAssignment = useCallback((report) => {
    setNotification({
      reportId: report.id,
  ...
  useEffect(() => {
    try {
      ...
      const isChargeEtude = (role) => {
        ...
        return (
          r.includes("charge d'étude") ||
          r.includes('charge_etude') ||
          r.includes('charge-etude') ||
          r.includes('technical_review')
        );
      };
  ...
  const getInitials = (name) => {
    if (!name || name.trim() === '') return user?.email?.[0]?.toUpperCase() || 'CE';
    ...
    <div className="ce-dashboard">
      ...
      {notification && (
        <div style={{
          ...
          <div style={{ fontWeight: 600 }}>{user?.full_name || user?.username || user?.email || 'Chargé d\'Étude'}</div>
      ...
      <div className="ce-container">
        ...
        <div className="ce-profile-section">
          <div className="ce-profile-header">
            ...
            <div className="ce-profile-info">
              <h3>{user?.full_name || user?.username || user?.email || 'Chargé d\'Étude'}</h3>
        ...
        <div className="ce-section">
          ...
          {assignedReports && assignedReports.length > 0 && (
            <div className="ce-filter-tabs">
              ...
              <button 
                className={`ce-filter-tab ${filterStatus === 'déposé'? 'active' : ''}`}
                onClick={() => setFilterStatus('déposé')}
              >
                En Attente ({stats.pending})
              </button>
              <button 
                className={`ce-filter-tab ${filterStatus === 'assigné' ? 'active' : ''}`}
                onClick={() => setFilterStatus('assigné')}
              >
                En Analyse ({stats.inProgress})
              </button>
              <button 
                className={`ce-filter-tab ${filterStatus === 'validé' ? 'active' : ''}`}
                onClick={() => setFilterStatus('validé')}
              >
                Vérifiés ({stats.verified})
              </button>
              <button 
                className={`ce-filter-tab ${filterStatus === 'clôturé' ? 'active' : ''}`}
                onClick={() => setFilterStatus('clôturé')}
              >
        ...
        {/* ✅ MODIFICATION: Added loading state + submitToResponsable in onValidate */}
        {selectedReport && (
          <div className="ce-section">
            <h2 className="ce-section-title">
              🔬 Examen Technique — {selectedReport.organism_name || selectedReport.company_name}
            </h2>
            {loadingReport ? (
              <div className="ce-loading">Chargement des données du rapport...</div>
            ) : (
              <TechnicalReviewInterface 
                report={reportWithUrl}
                onValidate={async (reportId) => {
                  const result = await submitToResponsable(reportId);
                  if (result.success) {
                    showToast('✅ Rapport soumis au responsable !');
                    refetch();
                  }
                  return result;
                }}
