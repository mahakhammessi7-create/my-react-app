  ...
  // ✅ À L'INTÉRIEUR du composant, après useState(...)
const getPublicPdfUrl = useCallback((rep) => {
  ...
  if (rep?.file_path) {
    const { data } = supabase.storage
      .from('audit-reports') // ← vérifie le nom exact de ton bucket Supabase
...
const reportWithUrl = selectedReport ? {
  ...selectedReport,
  file_url: getPublicPdfUrl(selectedReport),
    ...
    try {
      ...
      // Only call fetchChargesEtude ONCE — guard with ref
    ...
    try {
      ...
      // Write status directly to Supabase — no REST backend needed
      const { error } = await supabase
        .from('reports')
        .update({ status: 'in_progress' })
        .eq('id', reportId)
        .eq('assigned_to', user?.id); // safety: only update if assigned to this user
      ...
      // (refetch will also sync it via polling)
